...
const ChartSettings = ({ open, onClose, currentSettings, onSave }) => {
    ...
    return (
        <Modal open={open} onClose={onClose} title="Chart Settings">
            <div className="flex flex-col gap-4 px-4 text-(--text)">

                {/* Section: Appearance */}
                <div className="flex flex-col gap-2 text-(--text)">
                    <h3 className="text-md font-bold border-b border-(--graphite) pb-1">Appearence</h3>
                    ...
                    <div className="flex justify-end items-center">
                        <button onClick={resetBackground} className="px-3 py-0.5 rounded-sm bg-gray-700 hover:opacity-70 cursor-pointer">Default</button>
                    </div>
                </div>
                 <div className="flex flex-col gap-2">
                    <h3 className="text-md font-bold border-b border-(--graphite) pb-1">Grid</h3>
                    <div className="flex justify-between items-center">
                ...
                <div className="flex flex-col gap-2 mt-2">
                    <h3 className="text-md font-bold border-b border-(--graphite) pb-1">Candles</h3>
                    
                    <div className="flex justify-between items-center">
                        <label className="text-sm">Up Color</label>
                    ...
                    <div className="flex justify-between items-center">
                        <label className="text-sm">Down Color</label>
                        <input type="color" value={localSettings.candleDownColor} 
                               onChange={e => handleChange("candleDownColor", e.target.value)} 
                               className="h-10 w-10 cursor-pointer rounded-md border-4 border-(--graphite) hover:border-(--primary)"/>
                    </div>
                    <div className="flex justify-end items-center">
                        <button onClick={resetCandles} className="px-3 py-0.5 rounded-sm bg-gray-700 hover:opacity-70 cursor-pointer">Default</button>
                    </div>
                </div>

                {/* Section: Tools */}
                 <div className="flex flex-col gap-2">
                    <h3 className="text-md font-bold border-b border-(--graphite) pb-1">Tools</h3>
                    <div className="flex justify-between items-center">
                        <label className="text-sm">Magnet Mode</label>
                    ...
                    <div className="flex justify-between items-center">
                        <label className="text-sm">Clock Color</label>
